...
def extractBeers(source, templates, target_color=None, locked_position=False):
    ...
    for i in range(1, len(beers_binary_samples)):
        beers_binary = beers_binary & beers_binary_samples[
            i]  # logical AND operation performed on every binary image received from every passed template
        ...
        for blob in blobs:
            ...
            if blob.area > 0:  # some threshold to eliminate noise
                ...
                if distance < 30:  # some other threshold
                    ...
                    end_point_y = beer.center[0] + 40 if beer.center[0] + 40 < beer_area.shape[0] else beer_area.shape[
                        0]
                    end_point_x = beer.center[1] + 40 if beer.center[1] + 40 < beer_area.shape[1] else beer_area.shape[
                        1]
